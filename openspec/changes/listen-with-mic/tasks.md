## 1. Implementation
- [ ] 1.1 Add Kconfig options in `main/Kconfig.projbuild`:
...
- [ ] 1.2 Add matching defaults in `sdkconfig.defaults` (per project rule).
- [ ] 1.3 Create `main/streaming/streaming_state.{c,h}` for shared mutex/refcount, client
      flags, and video/audio failure flags.
- [ ] 1.4 Add `main/streaming/pcm_audio_stream.c` (and header) with:
...
- [ ] 1.5 Register `/audio` handler that streams raw PCM with `Content-Type: audio/pcm`,
      `Transfer-Encoding: chunked`, `Access-Control-Allow-Origin: *`, `Cache-Control: no-cache`.
- [ ] 1.6 Rename `/stream` to `/video` in code, logs, docs, and existing open change tasks
      (notably `openspec/changes/flip-video-vertically/tasks.md`).
- [ ] 1.7 Update `/video` handler to use shared lifecycle state, allow one `/video` + one
      `/audio` concurrently, and return 503 for extra clients.
- [ ] 1.8 Set `httpd_config_t.max_open_sockets = 2` and verify
...
- [ ] 1.9 Update `docs/manual-test-plan.md` with audio validation steps:
