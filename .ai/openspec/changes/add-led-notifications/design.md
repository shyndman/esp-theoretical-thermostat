...
- The strip is six WS2812-class pixels with an RGBW subpixel layout. We will rely on Espressif's `led_strip` component (RMT backend) to generate the timings, configured for 10 MHz and GRBW ordering on `CONFIG_THEO_LED_STRIP_GPIO`.
...
- If initialization succeeds and LEDs are enabled, `thermostat_led_status_booting()` starts the 0.33 Hz pulse immediately, bypassing quiet-hours checks until SNTP delivers time (or we exit boot). Once the clock is valid, subsequent LED requests go through the shared quiet-hours helper.
...
2. **`thermostat_led_status`**: high-level state machine that listens to boot lifecycle + HVAC dataplane events.
  ...
  - HVAC path: MQTT dataplane toggles `thermostat_led_status_set_hvac(bool heating, bool cooling)`. Heating wins when both true (defensive), with cooling selected only when heating is false. **TODO:** finalize which component surfaces these heating/cooling signals for LEDs; current UI view-model pathways may not be ideal, so we'll resolve this in a follow-up discussion before implementation.
