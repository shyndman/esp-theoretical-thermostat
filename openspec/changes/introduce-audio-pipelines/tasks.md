# Tasks
* [x] Add a `choice` block to `main/Kconfig.projbuild` for selecting the audio pipeline (`BSP codec` default vs `MAX98357 I2S amp`) plus the extra pin configs needed by the MAX path; update `sdkconfig.defaults` if necessary.
