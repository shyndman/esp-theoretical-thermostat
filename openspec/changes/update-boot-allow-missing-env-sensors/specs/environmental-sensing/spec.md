...
### Requirement: Environmental Topic Identity Initialization
The firmware SHALL initialize and make available the Theo-owned identity strings required to build MQTT topics for this device (Theo base topic, device slug, and device friendly name) without requiring environmental sensor hardware.

This identity initialization MUST complete before any code path that subscribes/publishes to Theo-owned topics can run, including MQTT connection handlers.

The identity strings MUST be derived from the configuration fields defined in `thermostat-connectivity` (`CONFIG_THEO_DEVICE_SLUG`, `CONFIG_THEO_DEVICE_FRIENDLY_NAME`, `CONFIG_THEO_THEOSTAT_BASE_TOPIC`) using the same normalization rules as other subsystems.

#### Scenario: MQTT connect happens before env hardware init stage
- **GIVEN** the boot sequence starts MQTT before starting environmental sensor hardware initialization
- **WHEN** the MQTT dataplane processes `MQTT_EVENT_CONNECTED` and attempts to subscribe to the device command topic using the Theo base topic
- **THEN** the Theo base topic is non-empty and the command topic subscription is not skipped due to missing identity initialization.

## MODIFIED Requirements

### Requirement: Environmental Sensor Hardware Initialization
The firmware SHALL initialize an ESP-IDF I2C master bus dedicated to the onboard environmental sensors during boot, using FireBeetle 2 GPIO7 for SDA and GPIO8 for SCL by default (both at 3.3 V with existing external pull-ups). The build MUST include `k0i05/esp_ahtxx^1.2.7` (for AHT20 temperature/humidity) and `k0i05/esp_bmp280^1.2.7` as managed component dependencies and create one handle per sensor over the shared bus. Sensor bring-up SHALL occur before the splash screen dismisses. If either sensor fails initialization, the firmware MUST report the failure on the splash screen as an error (red status line identifying the failed sensor) and continue booting to the main UI.

Implementation notes (non-normative, verified against the component documentation and current repo usage):
- AHT20 is accessed via `k0i05/esp_ahtxx` and the `ahtxx.h` API, typically:
  - `esp_err_t ahtxx_init(i2c_master_bus_handle_t bus, const ahtxx_config_t *cfg, ahtxx_handle_t *out_handle)`
  - `esp_err_t ahtxx_get_measurement(ahtxx_handle_t handle, float *out_temperature_c, float *out_relative_humidity)`
- BMP280 is accessed via `k0i05/esp_bmp280` and the `bmp280.h` API, typically:
  - `esp_err_t bmp280_init(i2c_master_bus_handle_t bus, const bmp280_config_t *cfg, bmp280_handle_t *out_handle)`
  - `esp_err_t bmp280_get_measurements(bmp280_handle_t handle, float *out_temperature_c, float *out_pressure_pa)`
...
- **WHEN** `app_main` attempts to initialize the AHT20 via the `k0i05/esp_ahtxx` driver over the shared bus
...
#### Scenario: One sensor missing still disables all env telemetry
- **GIVEN** the BMP280 is not present on the bus at power-on
- **WHEN** the firmware initializes environmental sensors during boot
- **THEN** it reports a BMP280 init failure on the splash as a red error line
- **AND** it does not start the environmental sampling task
- **AND** it does not publish partial AHT20-only telemetry.
...
For boot-time init failure, the service MUST still publish retained Home Assistant discovery configs for all environmental entities (see the discovery requirement) so that Home Assistant can mark the entities unavailable.
...
- **WHEN** MQTT reports ready (or is already connected)
- **THEN** the service publishes retained `"offline"` for all four environmental per-entity availability topics
- **AND** it publishes retained Home Assistant discovery configs for those entities
...
The publish MUST occur as soon as MQTT is connected for the first time during the boot (or immediately if MQTT is already connected by the time environmental sensing initializes).
...
6. Home Assistant availability configuration using the multi-availability format (verified against Home Assistant MQTT Sensor + MQTT Discovery docs):
   ...
   - The payload MUST NOT include `availability_topic` when `availability` is present.
