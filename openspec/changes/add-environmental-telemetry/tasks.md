...
- [ ] Introduce Kconfig entries for the environmental I2C pins (defaults SDA=7/SCL=8), device slug (`CONFIG_THEO_DEVICE_SLUG`), friendly name (`CONFIG_THEO_DEVICE_FRIENDLY_NAME`), Theo-owned base topic, and sampling interval; document defaults in `docs/manual-test-plan.md`.
...
- [ ] Publish and retain the four Home Assistant discovery configs under `homeassistant/sensor/<slug>-theostat/<object_id>/config` with the specified device metadata, re-sending whenever metadata changes.
