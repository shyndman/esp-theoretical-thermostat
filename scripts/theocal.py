...
# ]
# ///
"""Send a radar calibration command to the configured thermostat."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Send radar_calibrate command to the thermostat.",
    )
...
def resolve_config(args: argparse.Namespace, config_values: Mapping[str, str]) -> CalibrateConfig:
    ...
    return CalibrateConfig(host=host, port=port, path=path, topic=topic)
...
def main() -> int:
    ...
    # Use a flag to wait for publish completion
    published = False
    ...
    try:
        ...
        print(f"Publishing 'radar_calibrate' to {config.topic}...", file=sys.stderr)
        info = client.publish(config.topic, "radar_calibrate", qos=0)
        ...
        print(f"theocal: {exc}", file=sys.stderr)
        return 1
    finally:
        client.disconnect()
