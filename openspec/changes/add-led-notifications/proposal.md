...
## What Changes
- Introduce a WS2812 (RGB) LED service that initializes a 50-pixel strip via the ESP-IDF `led_strip` RMT backend on `CONFIG_THEO_LED_STRIP_GPIO` (default GPIO 33), exposes pulse/solid/off APIs with fixed fade timings, and clamps brightness to a single RGB color across all pixels.
